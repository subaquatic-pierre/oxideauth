...
impl<D: DbExecutor, C: CacheExecutor> ServiceRegistry<D, C> {
    ...
    pub fn new(sm: Arc<StoreManager<D>>, cm: Arc<CacheManager<C>>) -> Self {
        ...
        let permission = Arc::new(PermissionService::new(sm.clone(), workspace.clone(), cm.clone()));
        let role       = Arc::new(RoleService::new(sm.clone(), workspace.clone(), permission.clone(), cm.clone()));
        let account    = Arc::new(AccountService::new(sm.clone(), cm.clone(), workspace.clone()));
        ...
        let credential = Arc::new(CredentialService::new(sm.clone(), workspace.clone(), account.clone()));
        let membership = Arc::new(MembershipService::new(sm.clone(), cm.clone(), workspace.clone(), account.clone(), role.clone()));
        let client     = Arc::new(ClientService::new(sm.clone(), workspace.clone(), cm.clone()));
        let auth       = Arc::new(AuthService::new(sm.clone(), cm.clone(), account.clone(), token.clone(), config));

        Self {
            sm, cm,
            workspace, role, permission,
            account, project, token,
            credential, membership, client,
            auth,
        }
    }
}
